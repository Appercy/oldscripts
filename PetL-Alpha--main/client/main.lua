...
if IsEntityAVehicle(hitEntity) then
    local auto = ECM:AddItem(0, Translation[Config.Locale]['GetIncar'], function()
        ...
        TaskEnterVehicle(idd, hitEntity, 20000, 0, 16, 16, 0)
    end)
...
if Config.Yeet then
if not IsPedAPlayer(hitEntity) and idd ~= nil and not IsEntityAPed(hitEntity) and not IsEntityAVehicle(hitEntity) then
    local lauf = ECM:AddItem(0, "yeet", function()
        local pos = GetEntityCoords(idd, true)
        ApplyForceToEntity(idd, 0, 0, 0, 100, 0, 0, 100, 0, false, false, true, false, true)
    end)
end
end
